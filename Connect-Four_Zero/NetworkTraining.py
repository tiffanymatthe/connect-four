...
class NetworkTraining(object):
    ...
    def alphazero(config: C4Config):
        ...
        for _ in range(config.num_actors):
            NetworkTraining.launch_job(NetworkTraining.run_selfplay, config, storage, replay_buffer)
    ...
    def run_mcts(config: C4Config, game: C4Game, network: Network):
        ...
        while node.expanded():
            ...
            NetworkTraining.backpropagate(search_path, value, scratch_game.to_play())
    ...
    def train_network(config: C4Config, storage: SharedStorage,
        ...
        for i in range(config.training_steps):
            ...
            NetworkTraining.update_weights(optimizer, network, batch, config.weight_decay)
